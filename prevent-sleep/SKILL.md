...
The sleep-prevention hold exists only between `start` and `stop`. Nothing is requested before `start` or after `stop`; no power settings are modified; and outside the hold window the machine follows its normal power plan. Start the hold only when the task work actually begins, and stop it in the final step of the turn, success or failure. The self-release paths (Codex process exit, `-MaxHours` watchdog) are crash safety nets only, never the normal release path.
